...
def warmup_model(tag: str) -> None:
    ...
    payload = {
        "model": tag,
        "prompt": "How many people are in this image? Reply with just a number.",
        "images": [tiny_b64],
        "stream": False,
        "options": {"temperature": 0},
    }
    print(f"  [warm-up] loading {tag} into GPU …", end=" ", flush=True)
    t0 = time.time()
    try:
        requests.post(f"{OLLAMA_URL}/api/generate", json=payload, timeout=450)
    except Exception:
